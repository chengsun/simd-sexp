...
let%expect_test _ =
  let print_io test_string =
    ...
    let n =
      Simd_sexp.extract_structural_indices ~input ~output ~output_index:0 ~start_offset:0
    in
    ...
    in
    for i = 0 to n - 1 do
      Simd_sexp.State.process state input (Int64.to_int_exn output.{i})
    done;
    Simd_sexp.State.process_eof state input;
    printf "\n\n"
  in
  print_io {|foo|};
  print_io {|foo bar|};
